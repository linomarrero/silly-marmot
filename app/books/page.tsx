  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 pb-10">
      ...
      <section className="relative mt-6 overflow-hidden rounded-2xl border border-[#ddd3c5] bg-[#e3eadf] p-5 pb-8 md:p-8 md:pb-12">
        ...
        <div className="relative hidden min-h-[760px] lg:min-h-[820px] md:block">
          {books.map((book) => (
            <motion.button
              key={book.id}
              type="button"
              onClick={() => scrollToBook(book.id)}
              className={`absolute z-10 w-[32vw] min-w-[200px] max-w-[340px] rounded-xl border border-[#e1d7cb] bg-white p-5 text-left lg:max-w-[380px] lg:p-6 ${book.position}`}
              style={{ rotate: `${book.rotation}deg` }}
              whileHover={{ rotate: 0, scale: 1.05, zIndex: 50, boxShadow: "10px 14px 34px rgba(100,80,60,0.2)" }}
